...
class Car {
   ...
   */
  drive(distance) {
    const gasUsed = Math.round((distance/this.mpg) * 100)/100
    if(gasUsed > this.fuel){
      const actualDistance = mpg * this.fuel
      return this.odometer+actualDistance
    }
    this.fuel -= gasUsed
   ...
   */
  refuel(gallons) {
    if(gallons > this.tank || this.fuel === this.tank){ 
      this.fuel = this.tank
